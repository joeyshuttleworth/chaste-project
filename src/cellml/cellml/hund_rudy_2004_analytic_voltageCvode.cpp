...
double Cellhund_rudy_2004_analytic_voltageFromCellMLCvode::CalculateAnalyticVoltage(){
  Ca_i_tot = Ca_i*(1{dimensionless}+trpn_max/(K_mtrpn+Ca_i)+cmdn_max/(K_mcmdn+Ca_i));
  Ca_jsr_tot = Ca_jsr*(1{dimensionless}+csqn_max/(K_mcsqn+Ca_jsr));
  Ca_ss_tot = Ca_ss*(1{dimensionless}+BSRmax/(Ca_ss+KmBSR)+BSLmax/(Ca_ss+KmBSL));
  vcell = 1000{m2u}*pi*a*a*l;
  ageo = 2{dimensionless}*pi*a*a+2{dimensionless}*pi*a*l;
  Acap = ageo*2{uF_per_cm2};
  vmyo = vcell*0.68{dimensionless};
  vmito = vcell*0.26{dimensionless};
  vsr = vcell*0.06{dimensionless};
  vnsr = vcell*0.0552{dimensionless};
  vjsr = vcell*0.0048{dimensionless};
  vss = vcell*0.02{dimensionless};

  V = F*vmyo/(Acap*Cm)*(-Cl_i+K_i+Na_i+2{dimensionless}*Ca_i_tot+2{dimensionless}*Ca_ss_tot*vss/vmyo+2{dimensionless}*Ca_jsr_tot*vjsr/vmyo+2{dimensionless}*Ca_nsr*vnsr/vmyo)-mIntegrationConstant;
}
                ...
                pIntracellularStimulus)
    {
        ...
        mIntegrationConstant = 0;

        mIntegrationConstant = voltage - CalculateAnalyticVoltage(mStateVariables);
    }
    ...
    void Cellhund_rudy_2004_analytic_voltageFromCellMLCvode::EvaluateAnalyticJacobian(double var_chaste_interface__Environment__time, N_Vector rY, N_Vector rDY, CHASTE_CVODE_DENSE_MATRIX rJacobian, N_Vector rTmp1, N_Vector rTmp2, N_Vector rTmp3)
    {
