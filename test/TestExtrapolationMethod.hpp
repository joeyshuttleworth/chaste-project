...
public:
  void TestTusscherSimulation()
  {
    ...
    boost::shared_ptr<Cellohara_rudy_cipa_v1_2017FromCellMLCvode> p_model(new Cellohara_rudy_cipa_v1_2017FromCellMLCvode(p_solver, p_stimulus));
    ...
    SmartSimulation smart_simulation(p_model, 500);
    ...
    const double initial_v = p_model->CalculateAnalyticVoltage();
    ...
    std::vector<double> states = p_model->GetStdVecStateVariables();
    ...
    // std::cout << "Pace MRMS between solutions is " << CalculatePaceMrms(p_model, simulation.GetStateVariables(), smart_simulation.GetStateVariables(), 500, duration) << "\n";

    // final_states.close();

    // simulation.WriteToFile("sim-finalpace");
